...
cout << "\tloading " << file << endl;
	textureIndex = TextureManager::get(file);
}
...
void Obj::load(string filename) {
    ...
    if(objfile.is_open()) {
		...
		string fullPath =  directory + string(materialName);
		cout << "loading " << fullPath << endl;
		...
		if(mtlfile.is_open()) {
			...
			if (sscanf (mtlline.c_str(), "newmtl %s", name) == 1) {
			    ...
			    material.textures[material.textures.size() - 1].load(name);
			}
		    }
		    mtlfile.close();
		}
    ...
    for(int g = 0; g < groups.size(); g++) {
	   for(int t = 0; t < material.textures.size(); t++)
		    if(material.textures[t].name == groups[g].material) {
                            glBindTexture(GL_TEXTURE_2D, material.textures[t].textureIndex);
                            glTexEnvi(GL_TEXTURE_FILTER_CONTROL_EXT, GL_TEXTURE_LOD_BIAS_EXT, 7);
                    }
